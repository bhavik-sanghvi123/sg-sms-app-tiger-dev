define(["postmonger"], function (Postmonger) {
  ...
  function save() {
    ...
    let brandDropdownSelectedVal =
      brandDropdown.options[brandDropdown.selectedIndex].value;

    console.log("Brand Name Dropdown Val >>> ", brandDropdownSelectedVal);

    var regexExp = /\%%.*?\%%/gi;
    var regexedStr = textMessageValue.replace(
      regexExp,
    ...
    console.log(
      "THIS IS THE SCHEMA FOR PHONE ATR >>",
      `{{Event.${eventDefinitionKey}."${phoneAttrSchema}"}}`
    );
  ...
  function initialLoad(data) {
    ...
    try {
      $("#textMessage").val(
        savedData.arguments.execute.inArguments[0].textMessage
      );
      document.getElementById("textMessage").dataset.hiddenmsgid =
        savedData.arguments.execute.inArguments[0].selectedMsgID;
      document.getElementById("textMessage").dataset.hiddenshortmsg =
        savedData.arguments.execute.inArguments[0].shortMsg;
      $("#campaignName").val(
        savedData.arguments.execute.inArguments[0].campaignName
      );
      document.getElementById("brandNames").value =
        savedData.arguments.execute.inArguments[0].brandNames;
    } catch (e) {
        ...
        for (var i = 0; i < objArr.length; i++) {
          if (objArr[i].assetType.displayName == "JSON Message") {
            ...
            if (objArr[i].views.sMS !== undefined) {
              str =
                objArr[i].views.sMS.meta.options.customBlockData[
                  "display:message"
                ];
  ...
  //Javascript function for removing a class 'card' from the individual card when selected
